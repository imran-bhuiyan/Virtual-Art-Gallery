...
public class adminCustomerAddController extends BaseController {
    ...
    void addTodatabase(ActionEvent event) {
        ...
        try {
            addCustomer(fullName, emailAddress, passwordText, birthDate);
        }
        catch (SQLException e) {
            showAlert(Alert.AlertType.ERROR, "Database Error", "An error occurred while registering.");        }
    }

    private void addCustomer(String fullName, String email, String password, LocalDate birthDate) throws SQLException {

        String query = "INSERT INTO users (name, email, password, dob, role) VALUES (?, ?, ?, ?, 'customer')";
